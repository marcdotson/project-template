...
def run_regressions():
  ...
  def sim_data(n, beta_0, beta_condition, beta_odometer, beta_vehicle_age, state_effects, sigma):
    ...
    # Map each state to its numeric effect. Default to 0 if not specified.
    state_effect_array = np.array([state_effects.get(s, 0) for s in state])

    # Simulate the price outcome
    price = beta_0 + beta_condition * condition + beta_odometer * odometer + beta_vehicle_age * vehicle_age + state_effect_array + error

    # Return the output
    return price, condition, odometer, vehicle_age, state

  # Call the function and save as an array
  # We provide a dictionary for state_effects (e.g., CA adds $1500 to the price)
  data_arr = sim_data(n = 100, beta_0 = 5000, beta_condition = 32, beta_odometer = 24000, beta_vehicle_age = 20, state_effects = {"CA": 1500}, sigma = 5)
